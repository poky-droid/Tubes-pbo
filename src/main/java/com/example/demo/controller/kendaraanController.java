...
public class kendaraanController extends BaseController {
    ...
    public String detailKendaraanAdmin(@PathVariable("id") Long id, Model model, HttpSession session) {
        if (!isOwner(session)) return "redirect:/login?accessDenied=true";
        ...
        return "admin/kendaraan-detail";
    }

    @PostMapping("/admin/kendaraan/edit/{id}")
    public String editKendaraan(
        ...
        // Cek dulu kendaraan ada & ambil jenisnya saat ini (jenis tidak ikut diubah)
        String jenisSekarang;
        try {
            jenisSekarang = jdbcTemplate.queryForObject(
                "SELECT CASE WHEN m.id_kendaraan IS NOT NULL THEN 'mobil' " +
                "            WHEN mo.id_kendaraan IS NOT NULL THEN 'motor' END " +
                "FROM kendaraan k " +
                "LEFT JOIN mobil m ON k.id_kendaraan = m.id_kendaraan " +
                "LEFT JOIN motor mo ON k.id_kendaraan = mo.id_kendaraan " +
                "WHERE k.id_kendaraan = ?", String.class, id);
        } catch (Exception e) {
            redirectAttributes.addFlashAttribute("error", "Kendaraan tidak ditemukan.");
            return "redirect:/admin/kendaraan";
        }

        // ── Upload foto baru (opsional), validasi sama seperti saat tambah ──
        String namaFileBaru = null;
        if (foto != null && !foto.isEmpty()) {
            String contentType = foto.getContentType();
            if (contentType == null || !contentType.startsWith("image/")) {
                redirectAttributes.addFlashAttribute("error", "File harus berupa gambar.");
                return "redirect:/admin/kendaraan/detail/" + id;
            }
            if (foto.getSize() > 5 * 1024 * 1024) {
                redirectAttributes.addFlashAttribute("error", "Ukuran foto maksimal 5MB.");
                return "redirect:/admin/kendaraan/detail/" + id;
            ...
            try {
                ...
                namaFileBaru = UUID.randomUUID().toString() + ekstensi;
                Files.write(Paths.get(UPLOAD_DIR + namaFileBaru), foto.getBytes());
            } catch (IOException e) {
                e.printStackTrace();
                redirectAttributes.addFlashAttribute("error", "Gagal mengunggah foto, silakan coba lagi.");
                return "redirect:/admin/kendaraan/detail/" + id;
        ...
        // ── Update tabel induk kendaraan ──
        if (namaFileBaru != null) {
            // foto baru diunggah -> hapus foto lama dari disk supaya tidak menumpuk file sampah
            try {
                String fotoLama = jdbcTemplate.queryForObject(
                    "SELECT foto FROM kendaraan WHERE id_kendaraan = ?", String.class, id);
                if (fotoLama != null && !fotoLama.isEmpty()) {
                    new File(UPLOAD_DIR + fotoLama).delete();
                }
            } catch (Exception e) { /* abaikan */ }

            jdbcTemplate.update(
                "UPDATE kendaraan SET merk=?, model=?, tahun=?, harga=?, status=?, warna=?, foto=? WHERE id_kendaraan=?",
                merk, model, tahun, harga, status, warna, namaFileBaru, id);
        } else {
            jdbcTemplate.update(
                "UPDATE kendaraan SET merk=?, model=?, tahun=?, harga=?, status=?, warna=? WHERE id_kendaraan=?",
                merk, model, tahun, harga, status, warna, id);
        }

        // ── Update tabel spesifik sesuai jenis kendaraan yang sudah ada ──
        if ("mobil".equalsIgnoreCase(jenisSekarang)) {
            jdbcTemplate.update(
                "UPDATE mobil SET mesin_mobil=?, jenis_mobil=?, transmisi_mobil=?, kapasitas_mobil=? WHERE id_kendaraan=?",
                mesinMobil != null ? mesinMobil : "-",
                jenisMobil != null ? jenisMobil : "-",
                transmisi != null ? transmisi : "-",
                kapasitasMobil != null ? kapasitasMobil : 0,
                id);
        } else if ("motor".equalsIgnoreCase(jenisSekarang)) {
            jdbcTemplate.update(
                "UPDATE motor SET cc=?, jenis_motor=?, kapasitas_tangki=? WHERE id_kendaraan=?",
                cc != null ? cc : 0,
                jenisMotor != null ? jenisMotor : "-",
                kapasitasTangki != null ? kapasitasTangki : 0.0,
                id);
        ...
        redirectAttributes.addFlashAttribute("success", "Kendaraan berhasil diperbarui.");
        return "redirect:/admin/kendaraan/detail/" + id;
    }
